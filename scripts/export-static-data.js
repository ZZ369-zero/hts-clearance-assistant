...
const chinaTariffs301SourceUrl = "https://hts.usitc.gov/reststop/file?filename=China+Tariffs&release=currentRelease";
const execFileAsync = promisify(execFile);

const syncSourceConfig = {
  hts: {
    ids: ["htsStatus", "chapter99", "chinaTariffs301"],
    minutes: 60
  },
  policyRules: {
    ids: ["policyRules"],
...
const sourceLabels = {
  ...
  chinaTariffs301: ["China 301 HTS mapping", "USITC China Tariffs", "USITC China Tariffs PDF mapped into searchable HTS rows, with sentinel checks for query hit coverage.", chinaTariffs301SourceUrl],
...
async function exportHts(manifest) {
  ...
  const chinaTariffs301 = await exportChinaTariffs301Snapshot(manifest);
  const chinaTariffs301Map = buildChinaTariffs301Map(chinaTariffs301);
  const chinaTariffs301Diagnostics = {
    expectedRows: 0,
    appliedRows: 0,
    anomalies: []
  };
  ...
  for (const [chapter] of manifest.chapters) {
    ...
    mergeChinaTariffs301IntoRows(rows, chinaTariffs301Map, chinaTariffs301Diagnostics);
  ...
  validateChinaTariffs301SearchCoverage(searchRows, chinaTariffs301, chinaTariffs301Diagnostics);
  ...
  finalizeChinaTariffs301SourceState(manifest, chinaTariffs301, chinaTariffs301Diagnostics);
}

async function exportChinaTariffs301Snapshot(manifest) {
  const snapshotPath = path.join(dataDir, "china-tariffs-301.json");
  const previous = await readJsonSafe(snapshotPath, null);
  let snapshot;

  try {
    const pythonCommand = process.env.PYTHON || "python";
    const parserPath = path.join(__dirname, "parse-china-tariffs-301.py");
    ...
    if (!previous?.entries?.length) {
      snapshot = {
        ...
        sourceUrl: chinaTariffs301SourceUrl,
        count: 0,
        mappedPrefixes: 0,
        entries: [],
        byHts: {},
      ...
      console.warn(`USITC China Tariffs export failed, keeping previous snapshot: ${error.message}`);
      snapshot = {
        ...previous,
...
function buildChinaTariffs301Map(snapshot) {
  const map = new Map();
  const byHts = snapshot?.byHts || {};
  for (const [hts, codes] of Object.entries(byHts)) {
    const digits = normalizeStaticHtsDigits(hts);
    const values = normalizeChina301Codes(codes);
    if (digits && values.length) {
      map.set(digits, values);
    }
  }

  for (const entry of snapshot?.entries || []) {
    const digits = normalizeStaticHtsDigits(entry.hts || entry.displayHts);
    const code = normalizeChina301Code(entry.chapter99);
    if (!digits || !code) {
      continue;
    }
    map.set(digits, [...new Set([...(map.get(digits) || []), code])].sort());
  }
  return map;
}

function mergeChinaTariffs301IntoRows(rows, chinaTariffs301Map, diagnostics) {
  for (const row of rows || []) {
    if (!row?.htsno) {
      continue;
    }
    const expectedCodes = applyKnownStaticChina301Overrides(row, getChinaTariffs301CodesForRow(row.htsno, chinaTariffs301Map));
    if (expectedCodes.length) {
      diagnostics.expectedRows += 1;
      const previousCodes = new Set(row.additionalDutyCodes || []);
      row.additionalDutyCodes = [...new Set([...(row.additionalDutyCodes || []), ...expectedCodes])].sort();
      if (expectedCodes.some((code) => !previousCodes.has(code))) {
        diagnostics.appliedRows += 1;
      }
      if (expectedCodes.some((code) => !(row.additionalDutyCodes || []).includes(code))) {
        diagnostics.anomalies.push({
          hts: row.htsno,
          issue: "missing-applied-code",
          expected: expectedCodes
        });
      }
    }
    row.additionalDutyCodes = applyKnownStaticChina301Overrides(row, row.additionalDutyCodes || []);
  }
}

function getChinaTariffs301CodesForRow(htsno, chinaTariffs301Map) {
  const digits = normalizeStaticHtsDigits(htsno);
  if (!digits || digits.startsWith("99") || !chinaTariffs301Map?.size) {
    return [];
  }
  const codes = [];
  for (let length = Math.min(10, digits.length); length >= 4; length -= 1) {
    const prefixCodes = chinaTariffs301Map.get(digits.slice(0, length));
    if (prefixCodes?.length) {
      codes.push(...prefixCodes);
    }
  }
...
function validateChinaTariffs301SearchCoverage(searchRows, snapshot, diagnostics) {
  const byDigits = new Map((searchRows || []).map((row) => [normalizeStaticHtsDigits(row.htsno), row]));
  for (const sentinel of snapshot?.sentinels || []) {
    const sourceCodes = normalizeChina301Codes(snapshot.byHts?.[sentinel.hts]);
    ...
    if (!expectedCode || !sourceCodes.includes(expectedCode)) {
      diagnostics.anomalies.push({
        ...
        issue: "source-sentinel-missing",
        expected: [expectedCode || sentinel.chapter99].filter(Boolean)
      });
      continue;
    }
...
function finalizeChinaTariffs301SourceState(manifest, snapshot, diagnostics) {
  const count = snapshot.count || snapshot.entries?.length || 0;
  const anomalySamples = (diagnostics.anomalies || [])
    .slice(0, 5)
    .map((item) => `${item.hts}: ${item.issue}${item.expected?.length ? ` (${item.expected.join(",")})` : ""}`);
  const hasWarning = Boolean(
    snapshot.warning
    || snapshot.retainedPreviousRows
    || count < 10000
    || anomalySamples.length
  );

  setSourceState(manifest, "chinaTariffs301", {
    count,
    mappedPrefixes: snapshot.mappedPrefixes || Object.keys(snapshot.byHts || {}).length,
    expectedRows: diagnostics.expectedRows,
    appliedRows: diagnostics.appliedRows,
    anomalyCount: diagnostics.anomalies.length,
    anomalySamples,
    sourceWarning: snapshot.warning || "",
    retainedPreviousRows: Boolean(snapshot.retainedPreviousRows),
    fetchedAt: snapshot.generatedAt || now,
    status: hasWarning ? "warning" : "ok",
    message: hasWarning
      ? `China Tariffs 301 映射需复核${anomalySamples.length ? `：${anomalySamples.join("；")}` : snapshot.warning ? `：${snapshot.warning}` : ""}`
      : "China Tariffs 301 映射已同步并通过查询命中校验"
  });
}
...
function mergeSources(existing, selected, counts) {
  ...
  for (const selectedScope of selected) {
    ...
    for (const id of config.ids) {
      ...
      const existingDetail = existingState.detail || {};
      const preserveNonOkStatus = existingState.status && existingState.status !== "ok";
      byId.set(id, {
        id,
        ...
        state: {
          ...existingState,
          id,
          status: preserveNonOkStatus ? existingState.status : "ok",
          message: preserveNonOkStatus ? existingState.message || "Static snapshot needs review" : "Static snapshot updated",
          lastSyncAt: now,
...
function countForSource(id, counts) {
  ...
  if (id === "chinaTariffs301") return counts.chinaTariffs301Rows || 0;
...
function normalizeChina301Code(value) {
  ...
  const digits = normalizeStaticHtsDigits(text);
...
function normalizeChina301Codes(values) {
  const list = Array.isArray(values) ? values : [values];
  return [...new Set(list.map(normalizeChina301Code).filter(Boolean))].sort();
}

function isStaticChina301DutyCode(code) {
  return Boolean(normalizeChina301Code(code));
}

function formatStaticHts(value) {
