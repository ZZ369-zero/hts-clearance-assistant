...
一个网页版 MVP：查询美国 HTS CODE 和税率、按章节浏览全品类、估算进口税金，并通过 USITC 官方 HTS 接口刷新数据。
...
当前哨兵覆盖 Section 122 Annex II 豁免、`8471.30.0100` 不计入 122、`6307.90.98.91` 不误继承 `9903.91.01`、相关电子类 Annex II 前缀、`3304100000` 的 AM7/FD2 认证提示、232 清单和 AD/CVD 快照。校验失败时 Actions 会停止提交和部署，避免后台数据刷新成功但页面业务结论仍然异常。

`npm run check:ustr-exclusions` 额外验证 301 排除规则及前端实际计算链路：`3923.21.00.95` 是 U.S. note 20(vvv)(iii)(8) 列名项，在中国原产及美国入境日期 `2024-06-15` 至 `2026-11-09`（含两端）时，`9903.88.03` 的 25% 由 `9903.88.69` 排除。页面默认按中国原产、当前美国东部日期估算，保留该税项及官方依据，但不计入小计或税金。此排除不影响普通关税和其他独立税项；需核商品描述/重量的其他排除仍保留待核状态。页面显示两份 USTR 官方通知链接，到期后恢复原有 301 计算，延期须重新核实更新。
